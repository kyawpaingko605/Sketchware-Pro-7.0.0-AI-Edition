...
import pro.sketchware.ai.AiErrorFixActivity;
import pro.sketchware.activities.livepreview.LiveCodeEditorActivity;
...
public class CompileLogActivity extends BaseAppCompatActivity {
    ...
    /** Holds the current error text so AI Fix and Live Editor can access it. */
    private String currentErrorText = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        binding.aiFixButton.setOnClickListener(v -> {
            if (currentErrorText == null || currentErrorText.isEmpty()) {
                SketchwareUtil.toast("No error log to analyze.");
                return;
            }
            AiErrorFixActivity.launch(this, currentErrorText);
        });

        // ── Live Code Editor button ────────────────────────────────────────
        binding.liveEditorButton.setOnClickListener(v -> {
            LiveCodeEditorActivity.launch(this, null, scId);
        });
    ...
    private void setErrorText() {
        ...
        currentErrorText = error; // store for AI Fix & Live Editor
    ...
    private void toggleMonospacedText(boolean isChecked) {
        ...
        binding.tvCompileLog.setTypeface(isChecked ? Typeface.MONOSPACE : Typeface.DEFAULT);
    }
